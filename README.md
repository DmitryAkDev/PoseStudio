...
* A **Vulkan-powered 3D viewport** with photoreal PBR shading, image-based lighting from HDR environments (`.hdr`/`.exr`), MSAA, HDR post-processing (bloom, ACES tonemapping, screen-space subsurface scattering), soft PCSS ground shadows, and eighteen selectable shade modes (PBR, textured, cartoon, matcap, clay, lighting-only, silhouette, four wireframe variants including hidden-line, and data views for albedo, ambient occlusion, roughness, specular, normals, and UVs).
...
* **Interactive posing** — grab any joint directly on the figure and drag; the part you grabbed lights up and the whole body follows (full-body IK, below). Ctrl+drag rotates just that one joint, or hold **X**, **Y**, or **Z** and roll the mouse wheel to dial it about that axis. Anatomical joint limits, an optional skeleton overlay (View → Show Skeleton), several figures posable in one scene (the selected one wears a Blender-style blue outline; click an object to select it, empty space to clear), a Ground button that drops the figure to the floor in free fall, pose save/load, and unified undo/redo.
* **Full-body IK** — drag any joint and the whole body follows: feet stay planted and weight-bearing, the figure auto-balances (stepping — even walking — when you move it beyond its stance), pulls beyond reach lift it to dangle under gravity, and releases settle as a short animation with the pose holding where you let go. Pin any joint (select it and press **P**, or right-click → Pin Joint) and it stays exactly put while you drag the rest of the body — plant a hand on a rail and crouch under it; pins save with the pose and undo like any edit. No joint can be pushed through the floor.
...
<table>
  <tbody>
    <tr>
      ...
      <td align="center" valign="top" width="14.28%"><a href="https://github.com/DmitryAkDev"><img src="https://avatars.githubusercontent.com/u/25522773?v=4?s=100" width="100px;" alt="DmitryAkDev"/><br /><sub><b>DmitryAkDev</b></sub></a><br /><a href="https://github.com/PoseStudio/PoseStudio/commits?author=DmitryAkDev" title="Code">💻</a> <a href="#platform-DmitryAkDev" title="Packaging/porting to new platform">📦</a></td>
    </tr>
  </tbody>
</table>
