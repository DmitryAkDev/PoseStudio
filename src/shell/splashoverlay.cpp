/**
 * @file splashoverlay.cpp
 * @brief Implementation of SplashOverlay. See splashoverlay.h for why it is a top-level tool
 *        window rather than a child overlay.
 */
...
void SplashOverlay::mousePressEvent(QMouseEvent *) {
    dismiss(); // fallback; in practice the app-wide filter intercepts the press first
}

void SplashOverlay::showEvent(QShowEvent *event) {
...
void SplashOverlay::syncGeometryToParent() {
