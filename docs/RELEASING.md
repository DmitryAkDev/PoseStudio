...
CI is the source of truth, but for debugging: build Release, run `windeployqt --release --no-translations --compiler-runtime` on a staged copy of the exe (+ `shaders/`, `Maquettes/`, `vulkan-1.dll` from `System32`, `LICENSE.txt`, and `licenses\` from `packaging/licenses/`), download/unzip the stock pack, then:
